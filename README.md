...
The `Dockerfile` builds a Linux service with the official MolScribe checkpoint included. `railway.json` selects that image and checks `/api/health`. The image runs one worker on the platform's `PORT`; allocate enough memory for CPU PyTorch inference and attach a persistent volume mounted at `/data` if job results must survive restarts. `REQUIRE_OCSR=1` makes a missing model fail startup instead of quietly serving empty SMILES. First verify the Railway service URL, then add `image2smiles.ai-spectra.cn` as its custom domain. Railway supplies a CNAME target and a separate ownership TXT record; both must be added in Cloudflare before the hostname works.
